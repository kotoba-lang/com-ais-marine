(ns ais-marine.main
  "Kotodama WASM entrypoint for the Ais_marine clean-room actor (L4) — Clojure port.
  ...
  facts (`ais_marine.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ais_marine")
(def tier "L4")
...
(def entity-specs
  [{:entity "Message"   :plural "messages"   :id-prefix "aismarin_mes"
    :fields [:messageType :repeatIndicator :mmsi]              :required [:messageType :repeatIndicator]
    :coerce {:messageType :int :repeatIndicator :int :mmsi :int}                       :refs {}}
   {:entity "PositionReport"      :plural "positionreports"      :id-prefix "aismarin_pos"
    :fields [:messageType :mmsi :navigationStatus :rateOfTurn :speedOverGround :longitude :latitude :courseOverGround :trueHeading :maneuverIndicator] :required [:messageType :mmsi]
    :coerce {:messageType :int :mmsi :int :navigationStatus :int :rateOfTurn :float :speedOverGround :float :longitude :float :latitude :float :courseOverGround :float :trueHeading :int :maneuverIndicator :int}          :refs {}}
   {:entity "BaseStationReport"     :plural "basestationreports"     :id-prefix "aismarin_bas"
    :fields [:messageType :mmsi :longitude :latitude :typeOfEpfd]        :required [:messageType :mmsi]
    :coerce {:messageType :int :mmsi :int :longitude :float :latitude :float :typeOfEpfd :int} :refs {}}
   {:entity "StaticVoyageData" :plural "staticvoyagedatas" :id-prefix "aismarin_sta"
    :fields [:messageType :mmsi :imoNumber :callSign :vesselName :shipType :draught :destination]          :required [:messageType :mmsi]
    :coerce {:messageType :int :mmsi :int :imoNumber :int :shipType :int :draught :float}                       :refs {}}
   {:entity "AidToNavigationReport"    :plural "aidtonavigationreports"    :id-prefix "aismarin_aid"
    :fields [:messageType :mmsi :typeOfAid :name :longitude :latitude :typeOfEpfd]  :required [:messageType :mmsi]
    :coerce {:messageType :int :mmsi :int :typeOfAid :int :longitude :float :latitude :float :typeOfEpfd :int}          :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"ais_marine.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ais_marine-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ais_marine-compat/ready)
